...
class Io
{
     ...
     * @param int $flags (FILE_SKIP_EMPTY_LINES)
     * @param StreamContext|null $context
     * @return string[]
     */
    public static function readLines($file, int $start = 0, ?int $count = null, int $flags = 0, ?StreamContext $context = null): array
    {
        Check::flags($flags, FILE_SKIP_EMPTY_LINES);
        ...
        error_clear_last();
        $result = $context !== null
            ? @file($file, $flags | FILE_IGNORE_NEW_LINES, $context->getResource())
            : @file($file, $flags | FILE_IGNORE_NEW_LINES);
